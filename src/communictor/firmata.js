...
MIDI_RESPONSE[REPORT_VERSION] = function (board) {
    ...
    board.emit('reportversion',board.firmware.name)
}
...
function Board (port) {
    ...
    this.on('queryfirmware', () => {
        ...
        if(board.firmware.name === HEX_NAME){
            this.queryCapabilities(() => {
                this.queryAnalogMapping(ready)
            })
        }else{
            board.emit('versionExpired')
        }
...
Board.prototype.buggy_motor_control = function (type, leftspeed,rightspeed) {
    let leftForward = 1
    if(leftspeed < 0) {
    ...
    const msg = [START_SYSEX, 0x0C, 1, 3, leftForward, rightForward, leftspeed & 0x7F, (leftspeed >> 7) & 0x7F, rightspeed & 0x7F, (rightspeed >> 7) & 0x7F, END_SYSEX]
    return writeToTransport(this, msg)
}


//小强音乐模式
Board.prototype.buggy_music_control = function (beats,leftspeed,rightspeed) {
    let leftForward = 1
    if(leftspeed < 0) {
        leftspeed = leftspeed * -1
        leftForward = 0
    ...
    const msg = [START_SYSEX, 0x0C, 6, beats , leftForward, rightForward, leftspeed & 0x7F, (leftspeed >> 7) & 0x7F, rightspeed & 0x7F, (rightspeed >> 7) & 0x7F, END_SYSEX]
    return writeToTransport(this, msg)
}

//音乐模式停止
Board.prototype.buggy_music_stop = function () {
    const msg = [START_SYSEX, 0x0C, 6, 0, 0, 0, 0, 0, 0, 0, END_SYSEX]
    return writeToTransport(this, msg)
}
...
Board.prototype.buggy_buzzer_song = function(song) {
    return new Promise((resolve, reject) => {
        this.bot_buzzer_song(song).then(ret =>{
            console.log("播放结束")
        })
        setTimeout(resolve, timeOfSongs[song])
    })
}
